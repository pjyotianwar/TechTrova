...
function Order() {
  ...
  const makepayment = async () => {
    const stripe = await loadStripe(
      "pk_test_51O2U5sSIl0WA5V8G9bBbzrRNRvMfQcsvhvUDFVt2d9Ck23T14thi7Pabp1dICuZCPRkhlEEcChLpzVx0zAco21qI00fFkzz5Fk"
    );
    ...
    <div className="Order_Container">
          ...
          filteredItems.map((item, index) => (
            <div key={index} className="container">
              ...
              <div className="second-cont">
                <p>
                  Brand-Name: <span>{item.name}</span>
                </p>
                <p className="BrandPrice">
                  Brand-Price: ₹<span>{item.price}</span>
                </p>
      ...
      {filteredItems.length > 0 && (
        <div className="amount">
              ...
              {filteredItems.map((item, index) => (
                <tr key={index} className="Item_amount">
                  <td>
                    {" "}
                    No of item{" "}
                    <span style={{ color: "#d63031" }}>({item.quantity})</span>
                    <br />
                  </td>
          ...
          <button onClick={makepayment} className="PaymentButton">
            Place your order
          </button>
        </div>
      )}
